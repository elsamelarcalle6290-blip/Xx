...
def _media_duration_us(path: str) -> int:
    try:
        ...
        logger.warning(f"鏃犳硶璇诲彇瑙嗛鏃堕暱锛屽皢娌跨敤妯℃澘鏃堕暱: {exc}")
        return 0
...
def _audio_duration_us(path: str) -> int:
    try:
        ...
        logger.warning(f"鏃犳硶璇诲彇闊抽鏃堕暱锛屽皢娌跨敤妯℃澘鏃堕暱: {exc}")
        return 0
...
def _make_text_material(text: str) -> dict:
            ...
            "styles": [
                {
                    ...
                    "font": {"path": "F:/鍓槧/JianyingPro/10.2.0.13877/Resources/Font/SystemFont/zh-hans.ttf", "id": ""},
                    "size": 15,
    ...
    return {
        ...
        "font_id": "",
        "font_name": "",
        "font_path": "F:/鍓槧/JianyingPro/10.2.0.13877/Resources/Font/SystemFont/zh-hans.ttf",
        "font_resource_id": "",
        "font_size": 15.0,
...
def export_jianying_draft_project(
    ...
    if not enabled or not draft_root or not template_dir:
        logger.info("鍓槧鑽夌宸ョ▼瀵煎嚭鏈惎鐢紝璺宠繃")
        return None
    ...
    if not os.path.isdir(template_dir):
        logger.warning(f"鍓槧鑽夌妯℃澘鐩綍涓嶅瓨鍦? {template_dir}")
        return None
    ...
    if not source_video:
        logger.warning("娌℃湁鎵惧埌鍙敤浜庣敓鎴愬壀鏄犺崏绋垮伐绋嬬殑瑙嗛鏃堕棿绾?)
        return None

    os.makedirs(draft_root, exist_ok=True)
    draft_folder_name = _safe_name(
        draft_name or f"NarratoAI_{datetime.now().strftime('%m鏈?d鏃%H%M%S')}"
    )
    ...
    if not target_video_path:
        logger.warning("瑙嗛鏃堕棿绾垮鍒跺け璐ワ紝鏃犳硶鐢熸垚鍓槧鑽夌宸ョ▼")
        return None
    ...
    if not os.path.isfile(draft_content_path) or not os.path.isfile(draft_meta_path):
        logger.warning("鍓槧鑽夌妯℃澘缂哄皯蹇呰鐨?JSON 鏂囦欢")
        return None
    ...
    logger.success(f"鍓槧鑽夌宸ョ▼宸插鍑? {project_dir}")
    return project_dir
...
def export_jianying_import_package(
    ...
    if not enabled or not export_root:
        logger.info("鍓槧瀵煎叆鍖呭鍑烘湭鍚敤锛岃烦杩?)
        return None
    ...
    finals_dir = os.path.join(package_dir, "01_鎴愮墖")
    timeline_dir = os.path.join(package_dir, "02_鏃堕棿绾跨礌鏉?)
    segment_video_dir = os.path.join(package_dir, "03_鍒嗘瑙嗛")
    segment_audio_dir = os.path.join(package_dir, "04_鍒嗘闊抽")
    segment_subtitle_dir = os.path.join(package_dir, "05_鍒嗘瀛楀箷")
    scripts_dir = os.path.join(package_dir, "06_鑴氭湰")
    ...
    readme_path = os.path.join(package_dir, "瀵煎叆璇存槑.txt")
    with open(readme_path, "w", encoding="utf-8") as fp:
        fp.write(
            "NarratoAI 鍓槧瀵煎叆鍖匼n\n"
            "1. 鎵撳紑鍓槧涓撲笟鐗堬紝鏂板缓鑽夌銆俓n"
            "2. 浼樺厛瀵煎叆 02_鏃堕棿绾跨礌鏉?涓殑瑙嗛鍜岄煶棰戯紝涔熷彲鐩存帴浣跨敤鍓槧鑽夌宸ョ▼瀵煎叆缁撴灉銆俓n"
            "3. 闇€瑕佹洿缁嗗垎鐨勫井璋冩椂锛屽啀瀵煎叆 03_鍒嗘瑙嗛銆?4_鍒嗘闊抽銆?5_鍒嗘瀛楀箷銆俓n"
            "4. 06_鑴氭湰 涓繚瀛樹簡鍘熷鑴氭湰鍜屾暣鐞嗗悗鐨勫垎娈垫竻鍗曪紝鏂逛究瀵圭収淇敼銆俓n"
        )

    logger.success(f"鍓槧瀵煎叆鍖呭凡瀵煎嚭: {package_dir}")
    return package_dir

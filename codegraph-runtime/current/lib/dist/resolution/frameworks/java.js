...
exports.springResolver = {
    ...
    resolve(ref, context) {
        ...
        if (ref.referenceName.endsWith(':prefix')) {
            ...
            const candidates = context.getNodesByKind('constant').filter((n) => (n.language === 'yaml' || n.language === 'properties')
            ...
            const best = candidates.reduce((a, b) => canonicalConfigKey(a.qualifiedName).length <= canonicalConfigKey(b.qualifiedName).length ? a : b);
        ...
        if ((ref.language === 'java' || ref.language === 'kotlin') &&
            ...
            const candidates = context.getNodesByKind('constant').filter((n) => n.kind === 'constant'
    ...
    extract(filePath, content) {
        ...
        if (cls)
            classPrefix = parseMappingPath(cls[1]);
        const VERB = {
        ...
        while ((match = reqRe.exec(safe)) !== null) {
            ...
            if (/^\s*(?:@[\w.]+(?:\([^)]*\))?\s*)*(?:public\s+|final\s+|abstract\s+|open\s+|data\s+|sealed\s+)*class\b/.test(after))
...
function isSpringConfigFile(filePath) {
 ...
 */
function extractSpringConfig(filePath, content) {
    const nodes = [];
    const isProperties = /\.properties$/i.test(filePath);
    const lang = isProperties ? 'properties' : 'yaml';
    const now = Date.now();
    const emitLeaf = (dottedKey, line, valueText) => {
        if (!dottedKey)
            return;
        nodes.push({
...
function extractSpringValueBindings(filePath, safe, lang, now, nodes, references) {
...
function parseMappingPath(args) {
    const m = args.match(/["']([^"']*)["']/);
    return m ? m[1] : '';
}
